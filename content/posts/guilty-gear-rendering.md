...
categories:
  - Computer Graphics
description: "GDC 2015 발표를 기반으로 한 길티기어 Xrd 카툰 렌더링 기법 분석. 셀 셰이딩, 버텍스 컬러 AO, UV 트릭을 Unity에서 직접 재현해보았다."
---

> 이 글은 GDC 2015 발표 "Guilty Gear Xrd's Art Style: The X Factor Between 2D and 3D"와 직접 수행한 분석 및 Unity 재현 결과를 바탕으로 작성되었다.
...
길티기어 Xrd는 3D 게임임에도 불구하고 2D 셀 애니메이션과 구분이 어려울 정도의 비주얼을 구현했다. 본 글에서는 이를 가능하게 한 핵심 기법들을 분석한다.
...
길티기어는 캐릭터마다 별도의 라이트 방향을 설정한다. 각 캐릭터가 가장 "예쁘게" 보이는 라이트 각도를 개별 지정하여, 월드 라이트와 분리하여 적용한다.
